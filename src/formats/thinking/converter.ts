...
//
// Recognized tags (see ./tags for the single shared vocabulary):
//   <thinking>...</thinking>  (also the `antml` prefix)
//   <think>...</think>
//   <reasoning>...</reasoning>
//   <thinking_mode>...</thinking_mode>
//
// Non-streaming only. Streaming is handled by ./stream (StreamingThinkingParser).

import { OPEN_TAG_SRC, CLOSE_TAG_SRC } from "./tags";
import { SYNTHETIC_THINKING_SIGNATURE } from "../wire/anthropic";

// Matches a full thinking/reasoning block. Case-insensitive, DOTALL body
// (`[\s\S]*?`), non-greedy so consecutive blocks are captured separately.
const THINKING_RE = new RegExp(
  `${OPEN_TAG_SRC}([\\s\\S]*?)${CLOSE_TAG_SRC}`,
  "gi",
);

// Matches an unclosed opening thinking/reasoning tag. Used after THINKING_RE has
// stripped all closed blocks to clean up raw tags models leave behind when stuck.
...
export class ThinkingConverter {
  ...
  parseThinking(text?: string): string[] {
    return this.parseThinkingRaw(text).filter((b) => b !== "");
  }
  ...
  private parseThinkingRaw(text?: string): string[] {
  ...
  applyToAnthropicMessage(
    ...
    for (const block of body.content) {
      ...
      // Emit thinking content blocks for each extracted block. `signature` is
