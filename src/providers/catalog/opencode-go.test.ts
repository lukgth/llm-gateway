...
// Every gateway-derived id uses the exact OpenCode CLI shape: a `ses_`/`msg_`
...
test("conflicting case variants are canonicalized and invalid caller ids re-derived", () => {
  const headers = {
    authorization: "Bearer go-key",
