...
function getField<T>(
  m: MiniMaxModelRemain,
  snake: keyof MiniMaxModelRemain,
  camel: keyof MiniMaxModelRemain,
): T | undefined {
...
function modelDisplayName(m: MiniMaxModelRemain): string {
  const raw = String(
    getField<string>(m, "model_name", "modelName") ?? "",
  ).trim();
...
function hasQuota(m: MiniMaxModelRemain): boolean {
  const intervalTotal =
    Number(
      getField(m, "current_interval_total_count", "currentIntervalTotalCount"),
    ) || 0;
  const weeklyTotal =
    Number(
      getField(m, "current_weekly_total_count", "currentWeeklyTotalCount"),
    ) || 0;
  const intervalPct = getField<number>(
    m,
    "current_interval_remaining_percent",
    "currentIntervalRemainingPercent",
  );
  const weeklyPct = getField<number>(
    m,
...
function makeWindow(
  ...
  const w: ProviderKeyUsageWindow = {
    id,
    label,
    used,
    limit,
    unit: total > 0 ? "credits" : "percent",
  };
    ...
    for (const { url, countMeansRemaining } of QUOTA_URLS) {
      ...
      if (apiCode !== 0) {
        ...
        return {
          windows: [],
          unavailable: true,
          message: `MiniMax quota API error: ${msg}`,
        };
      }

      const allRemains = (data.model_remains ??
        data.modelRemains ??
        []) as MiniMaxModelRemain[];
      ...
      for (const m of models) {
        ...
        const intervalRemainsMs =
          Number(getField(m, "remains_time", "remainsTime")) || undefined;
        ...
        const weeklyRemainsMs =
          Number(getField(m, "weekly_remains_time", "weeklyRemainsTime")) ||
          undefined;
      ...
      if (!windows.length) {
        return {
          windows: [],
          unavailable: true,
          message: "Could not build quota windows from MiniMax response.",
        };
      }
...
export const minimax = new MiniMaxAdapter({
  ...
  label: "MiniMax",
  blurb:
    "MiniMax M-series via OpenAI-compatible API. Subscription key required (not the pay-as-you-go key).",
  brand: "minimax",
