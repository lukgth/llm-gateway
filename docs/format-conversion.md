...
`messages`). It records the non-obvious _quirks_ - the small correctness rules
...
`AnthropicBlock`, etc.) - this file covers the _behavior_, wire-types.md
covers _what fields exist_.
...
> behavior lives elsewhere and composes _after_ conversion (see **Anthropic
...
> first-party _impersonation_ stack is intentionally **not** implemented (see
...
_9router:_ `concerns/toolCall.js` → `sanitizeToolId` / `generateToolCallId`,
...
_9router:_ `request/claude-to-openai.js` → `fixMissingToolResponsesOpenAI`.
...
- `json_schema`: _"You must respond with valid JSON that strictly follows this
  JSON schema: `json … ` Respond ONLY with the JSON object, no other text."_
- `json_object`: _"You must respond with valid JSON. Respond ONLY with a JSON
  object, no other text."_

_9router:_ `request/openai-to-claude.js` (response_format block).
...
_9router:_ `request/openai-to-claude.js` → `getContentBlocksFromMessage` (FILE
branch).
...
1. In an assistant turn containing `tool_use`, any text block appearing _after_
...
_9router:_ `formats/claude.js` → `fixToolUseOrdering`.
...
_9router:_ `formats/openai.js` normalizes `developer` → `system`; we fold it into
...
_9router:_ `formats/maxTokens.js` → `adjustMaxTokens` (we port the "required +
default" rule only).
...
| `medium`                  | `medium`           |
| `high`                    | `high`             |
| _(no equivalent)_         | `xhigh`            |
| _(no equivalent)_         | `max`              |
...
For generic OpenAI-compatible providers we never _fabricate_ a thinking config
...
_9router:_ `concerns/reasoning.js` → `extractReasoningText` (reads
...
> _textual_ `<thinking>…</thinking>` tag form on provider-native bodies before
...
- the docs also confirm a _real_ thinking block always carries `signature`,
  ...
  a valid Anthropic signature; it exists purely so the block's _shape_ matches
...
_9router:_ `concerns/usage.js` → `USAGE_EXTRACTORS.claude` + `buildUsage`, and
...
_9router:_ `concerns/finishReason.js` (`toOpenAIFinish`/`fromOpenAIFinish`,
`format:"claude"`).
...
_9router:_ `concerns/image.js` → `DATA_URI_RE`.
...
shape (post-conversion for a hop that converts _into_ Messages, pre-conversion
...
_catalog default_, not any provider-specific override.
...
- **Firecrawl web-tool loop:** runs _before_ the request hooks (unchanged); the
...
- `buildRoute` (adapter + transform-plan construction) - wrapped _inside_ the
